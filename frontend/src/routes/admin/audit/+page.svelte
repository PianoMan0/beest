<script lang="ts">
	...
	let anomalies = $state<Anomalies | null>(null);
	...
	function sev(s: 'none' | 'low' | 'high'): string {
		return s === 'high' ? 'red' : s === 'low' ? 'yellow' : 'green';
	}
</script>
				...
				<article class="paper">
					...
					<section class="sec">
						...
						<ActivityTimelineChart
							...
							onFilterChange={(info) => (filterInfo = info)}
							onComplete={(d) => (anomalies = d.error ? null : d.anomalies)}
						/>
					</section>
					...
					{#if anomalies}
						<section class="sec">
							...
							{#if anomalies.autoClicker.severity === 'none' && anomalies.macroTyper.severity === 'none' && anomalies.offRepo.count === 0}
								<div class="anom-row"><span class="dot green"></span> no anomaly signals</div>
							{:else}
								{#if anomalies.autoClicker.severity !== 'none'}
									<div class="anom-row"><span class="dot {sev(anomalies.autoClicker.severity)}"></span> <strong>Auto-clicker</strong> — {anomalies.autoClicker.evidence}</div>
								{/if}
								{#if anomalies.macroTyper.severity !== 'none'}
									<div class="anom-row"><span class="dot {sev(anomalies.macroTyper.severity)}"></span> <strong>Macro-typer</strong> — {anomalies.macroTyper.evidence}</div>
								{/if}
								{#if anomalies.offRepo.count > 0}
									<div class="anom-row"><span class="dot yellow"></span> <strong>Off-repo</strong> — {anomalies.offRepo.count} heartbeats</div>
								{/if}
							{/if}
						</section>
					{/if}
